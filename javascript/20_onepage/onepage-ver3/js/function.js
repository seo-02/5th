...
$(document).ready(function(){
  ...
  */
  ...
  const arrTopVal = [0,700,1400,2100,2800,3500];//각 section의 top값
  ...
  $mnu_1.on('click', function (evt) {
    pageMove(evt,arrTopVal[0]);
  });

  $mnu_2.on('click',function(evt){
    pageMove(evt,arrTopVal[1]);
  });
  
  $mnu_3.on('click',function(evt){
    pageMove(evt,arrTopVal[2]);
  });
  
  $mnu_4.on('click',function(evt){
    pageMove(evt,arrTopVal[3]);
  });
  
  $mnu_5.on('click',function(evt){
    pageMove(evt,arrTopVal[4]);
  });
  
  $mnu_6.on('click',function(evt){
    pageMove(evt,arrTopVal[5]);
  });
});
